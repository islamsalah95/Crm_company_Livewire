        ...
        <div class="row">
          ...
          <div class="mb-3 col-md-6 fv-plugins-icon-container">
            ...
            <div class="position-relative" data-select2-id="49">
                ...
                <datalist  id="browsers">
                    ...
                    @foreach ($jobs as $job)
                    <option value="{{ $job['job_ar'] }}"
                    @if ($job['id'] == $selectJob) selected @endif>
                    >{{ $job['job_ar'] }}</option>
                    @endforeach
                </datalist>
                {{ $myJob; }}
          ...
          <div class="mb-3 col-md-6 fv-plugins-icon-container">
            ...
            <div class="position-relative">
                <select id="companies" wire:model.live="companie_id" class="select2 form-select select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                    <option>Select companies</option>
                    @foreach ($companies as $companie)
                        <option value="{{ $companie['id'] }}"
                            @if ($companie['id'] == $selectCompanie_id) selected @endif>
                            {{ $companie['company_name'] }}
                        </option>
                    @endforeach
                </select>
            </div>
            @error('companie_id') <span class="error">{{ $message }}</span> @enderror
        </div>
